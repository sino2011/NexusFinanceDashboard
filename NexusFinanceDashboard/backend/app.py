    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if user and user['pass'] == password:
                # Generate a secure token containing the user's distinct database ID
                access_token = create_access_token(identity=str(user['id']))
                return jsonify({
                    "message": "Login successful",
                    "token": access_token, # Send this to your Vue/React frontend to save in localStorage
                    "user_id": user['id']
                }), 200
...
def fetch_info():   
    user_id = get_jwt_identity() # Extract logged-in user's ID safely
    connection = get_db_connection()
    try:
        with connection.cursor() as cursor:
            # SCOPED BY USER ID
            ...
            if annual_income <= 0: annual_income = 1
            ...
            return jsonify({
                "profile": {
                    "monthly_savings": round(monthly_savings, 2), "savings_rate": round(savings_rate, 1),
                    "balance_36mo": round(balance_36mo, 2), "completion_date": completion_date,
                    "total_contributed": total_contributed, "time_to_goal": timetogoal, "savings_target": savings_target
                },
                "base_savings": base_savings, "savings_history": history, "debt": debt_history, "monthly_averages_chart": monthly_averages_chart
            }), 200
    except Exception as e:
...
def extra_data():
    ...
    try:
        with connection.cursor() as cursor:
            # Storing records tagged with the specific user_id
...
def delete_trans(tx_id):
    ...
    try:
        with connection.cursor() as cursor:
            # Added check to make sure users can only delete their own rows
