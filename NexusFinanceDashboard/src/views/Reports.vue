<script setup>
    ...
    const getData = async() => {
        try {
            ...
            MidlleRowGraph.value.datasets[0].data = response.data?.deep_dive?.fixed_costs 
                || financialMetrics.value.deep_dive.fixed_costs
                
            MidlleRowGraph.value.datasets[1].data = response.data?.deep_dive?.variable_costs 
    ...
    onMounted(async () => {
        ...
        setTimeout(() => {
            ...
            const totalExtraSavings = savingsHistory.reduce((sum, value) => sum + value, 0);
        ...
        scales: {
            y: {
                ...
                ticks: { 
                    ...
                    callback: function(value) { return '$' + value; } // Adds context to the numbers
                }
            },
            x: {
                grid: { display: false },
        ...
        <div class="content-view">
            ...
            <div class="mainContainer">
                ...
                <div class="Total">
                    ...
                    </div> <p class="goal">Of ${{ financialMetrics.emergency_target }} target</p>
                    <div class="Pro-container">
                        <div class="Pro" :style="{ width: (financialMetrics.emergency_current / (financialMetrics.emergency_target || 1) * 100) + '%'}">
                            <span v-if="(financialMetrics.emergency_current / (financialMetrics.emergency_target || 1) * 100) > 10"></span>
                        </div>
                    </div>
                    ...
                    <div class="Savings">
                        ...
                        <p class="target">Of ${{ financialMetrics.savings_target }} Target</p>
                        <div class="Pro-container">
                            <div class="Pro" :style="{ width: (financialMetrics.current_savings / (financialMetrics.savings_target || 1) * 100) + '%'}">
                ...
                <div class="grid-transition-zone">
                    <div class="quotes" ref="tableRowRef" :class="{'animate-trigger' : isTableRowIntersecting}">
                        <div class="Right" :class="{shifted: isVisible}">
                            <div class="firstRow">
                                <div class="bar" :class="{shifted: isVisible}">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon" width="16" height="16" class=" text-[#A3A3B5]"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z"></path><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"></path></svg>
                                    <h3>The progress bar is the product.</h3>
                                    <p class="bord">Seeing a bar move 2% triggers the same dopamine loop as completing a task. Visible progress compresses perceived effort.</p>
                                    <h2>3.4x</h2>
                                    <p>higher completion with visible tracking</p>
                                </div>
                                <div class="Donut-card">
                                    <div class="w-8" style="background:rgba(163,163,181,0.1)"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon" width="16" height="16" class=" text-[#A3A3B5]"><path stroke-linecap="round" stroke-linejoin="round" d="M12 3c2.755 0 5.455.232 8.083.678.533.09.917.556.917 1.096v1.044a2.25 2.25 0 0 1-.659 1.591l-5.432 5.432a2.25 2.25 0 0 0-.659 1.591v2.927a2.25 2.25 0 0 1-1.244 2.013L9.75 21v-6.568a2.25 2.25 0 0 0-.659-1.591L3.659 7.409A2.25 2.25 0 0 1 3 5.818V4.774c0-.54.384-1.006.917-1.096A48.32 48.32 0 0 1 12 3Z"></path></svg></div>
                                    <h3>Known expenses should never be suprises.</h3>
                                    <p class="bord">Car registration. Holiday gifts. Annual subscriptions. Divide the total by 12 and automate monthly deposits into named buckets. The bill arrives — the money is already there.</p>
                                    <h2>94%</h2>
                                    <p class="stress">of financial stress is from predictable costs</p>
                                </div>
                            </div>
                            <div class="secRow">
                                <div class="auto">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon" width="16" height="16" class=" text-[#A3A3B5]"><path stroke-linecap="round" stroke-linejoin="round" d="m3.75 13.5 10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75Z"></path></svg>
                                    <h3>Remove willpower from the equation.</h3>
                                    <p class="bord">Auto-transfers scheduled for payday morning means the decision was made once. Every subsequent month costs zero cognitive load.</p>
                                    <h2>0</h2>
                                    <p>decisions after the first one</p>
                                </div>
                                <div class="savingsAcc">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon" width="16" height="16" class=" text-[#00C853]"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18 9 11.25l4.306 4.306a11.95 11.95 0 0 1 5.814-5.518l2.74-1.22m0 0-5.94-2.281m5.94 2.28-2.28 5.941"></path></svg>
                                    <h3>Your savings account might be costing you.</h3>
                                    <p class="bord">Moving $10,000 from 0.01% APY to 4.8% APY generates $479 in passive income annually. That's a flight, a month of groceries, or an extra mortgage payment.</p>
                                    <h2>+$479</h2>
...
<style scoped>
    ...
    :global(body) {
        ...
        background-image: 
          linear-gradient(rgba(163, 163, 181, .1) 1px, transparent 0),
          linear-gradient(90deg, rgba(163, 163, 181, .1) 1px, transparent 0);
    ...
    * {
    ...
    background: 
      ...
      #F4F5F7;
    background-size: 100% 150px, 48px 48px, 48px 48px, 100% 100%;
    ...
    .auto, .bar, .Donut-card, .savingsAcc {
        flex: 1;
    ...
    .auto h3, .bar h3, .Donut-card h3, .savingsAcc h3{
        font-family: DM Sans, sans-serif;
        color: #1A1A2E;
        line-height: 1.25;
        font-weight: 600;
    ...
    .auto p, .bar p, .Donut-card p, .savingsAcc p {
    ...
    .auto h2, .bar h2, .savingsAcc h2, .Donut-card h2 {
        color: #1A1A2E;
        font-family: JetBrains Mono, monospace;
        font-weight: 700;
        font-size: 1.875rem;
    ...
    .middle-card:hover, .Donut-card:hover, .auto:hover, .savingsAcc:hover, .bar:hover {
        transform: translateY(-15px);
        box-shadow: rgba(99, 102, 241, 0.4) 0px 10px 40px;  
    }
    ...
    @media (max-width: 768px) {
        ...
        .mainContainer {
            ...
            /* padding-left: 15px;
            padding-right: 15px; */
            padding-top: 50px; /* Give room for the floating menu button */
        }
    ...
    .Cards {
        display: flex;
        flex-direction: column; /* Ensure layout cards stack */
        align-items: center;
        width: 100%;
    }
    ...
    .grid-transition-zone {
        width: 100%;
        padding: 40px 15px; /* Clean mobile padding */
    }
    
    /* Force columns for the quote sections */
    .Right {
        flex-direction: column;
        width: 100%;
    }
    ...
    .auto, .bar, .Donut-card, .savingsAcc {
        ...
        margin: 15px 0  ;
        padding: 24px;
        /* Swap out the extreme side slide for a reliable vertical mobile entry */
        animation: list-appear-to 1s ease-out forwards  ; 
    }
    ...
    /* REMOVE the old '.quotes { opacity: 1  ; }' rule */
    /* REPLACE it with this contextual trigger override: */
    .quotes.animate-trigger {
        animation: mobileFadeUp 1.2s cubic-bezier(0.25, 1, 0.5, 1) forwards  ;
    }
    
    /* Ensure child cards don't play separate internal animations that conflict */
    .auto, .bar, .Donut-card, .savingsAcc {
        animation: none  ;
    }
